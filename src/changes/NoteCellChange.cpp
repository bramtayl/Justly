...
NoteCellChange::NoteCellChange(ChordsModel *chords_model_pointer_input,
    ...
    : QUndoCommand(parent_pointer_input),
      ...
      chord_number(chord_number_input), old_value(std::move(old_value_input)),
...
void NoteCellChange::undo() {
  ...
  chords_model_pointer->set_note_cell(note_number, note_chord_field, chord_number, old_value);
}

void NoteCellChange::redo() {
  Q_ASSERT(chords_model_pointer != nullptr);
  chords_model_pointer->set_note_cell(note_number, note_chord_field, chord_number, new_value);
}
