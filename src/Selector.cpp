...
void Selector::select(const QItemSelection &new_selection,
                      QItemSelectionModel::SelectionFlags command) {
  auto new_indices = new_selection.indexes();
  if (!(new_indices.isEmpty()) &&
      command.testAnyFlags(QItemSelectionModel::Select |
                           QItemSelectionModel::Toggle)) {
    auto previous_indices = selection().indexes();
    QModelIndex first_selected = new_indices.at(0);
    QModelIndex selection_parent;
    if (command.testFlag(QItemSelectionModel::Clear) ||
        previous_indices.isEmpty()) {
    ...
    return QItemSelectionModel::select(
        QItemSelection(first_selected, last_selected), command);
  }
