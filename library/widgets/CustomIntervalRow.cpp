...
void select_matching_preset(QComboBox& presets_box,
  ...
  for (auto index = 1; index < just_scale.size(); index++) {
    if (Interval(just_scale[index].ratio, 0) == value) {
      presets_box.setCurrentIndex(index - 1);
      return;
    }
  }
...
CustomIntervalRow::CustomIntervalRow(QUndoStack& undo_stack_input,
  ...
  const auto& just_scale = get_just_scale();
  // skip unison, which wouldn't change anything
  for (const auto& named_ratio : just_scale.mid(1)) {
    const auto ratio = named_ratio.ratio;
    QString text;
    QTextStream stream(&text);
    stream << named_ratio.name << " (" << ratio.numerator << "/"
           << ratio.denominator << ")";
    presets_box.addItem(text);
  }

  QObject::connect(&presets_box, &QComboBox::currentIndexChanged, this,
                   [&interval_editor_ref, &just_scale](int index) -> auto {
                     interval_editor_ref.setValue(
