...
static void test_model(Tester &tester, SongEditor *song_editor_pointer,
  ...
  for (auto column_number = 0; column_number < number_of_columns;
    ...
    delete_cell(song_editor_pointer, delete_index);
...
void Tester::run_tests() {
  ...
  test_model(
      *this, song_editor_pointer, chords_model,
      std::vector(
  ...
  trigger_edit_pitched_notes(song_editor_pointer, 1);
  test_model(
      *this, song_editor_pointer, pitched_notes_model,
      std::vector(
  ...
  trigger_edit_unpitched_notes(song_editor_pointer, 1);
  test_model(
      *this, song_editor_pointer, unpitched_notes_model,
      std::vector(
  ...
  play_cell(song_editor_pointer, chords_model.index(2, chord_interval_column));
...
// different encoding on windows or something
  ...
  export_to_file(song_editor_pointer, temp_export_file.fileName());
