...
public:
  ...
  Tester() : song_editor_pointer(make_song_editor()){};
  ~Tester() override { delete_song_editor(song_editor_pointer); };

  // prevent moving and copying
  Tester(const Tester &) = delete;
  auto operator=(const Tester &) -> Tester = delete;
  Tester(Tester &&) = delete;
  auto operator=(Tester &&) -> Tester = delete;
...
private slots:
  ...
  void test_to_strings() const {
    ...
    for (const auto &row : std::vector({
             ...
             ToStringRow(
                 {chords_model.index(6, chord_interval_column), "/2o1"}),
             ToStringRow(
                 {chords_model.index(7, chord_interval_column), "3/2o1"}),
  ...
  void test_unpitched_note_column_headers() const {
    ...
    test_column_headers(
        get_unpitched_notes_model(song_editor_pointer),
        std::vector({HeaderRow({unpitched_note_percussion_set_column,
                                "Percussion set"}),
                     HeaderRow({unpitched_note_percussion_instrument_column,
                                "Percussion instrument"}),
                     HeaderRow({unpitched_note_beats_column, "Beats"}),
                     HeaderRow({unpitched_note_velocity_ratio_column,
                                "Velocity ratio"}),
  ...
  void test_delete_pitched_note_cells() const {
    trigger_edit_pitched_notes(song_editor_pointer, 1);
    test_delete_cells(song_editor_pointer,
                      get_indices(get_pitched_notes_model(song_editor_pointer),
  ...
  void test_delete_unpitched_note_cells() const {
    ...
    test_delete_cells(
        song_editor_pointer,
        get_indices(get_unpitched_notes_model(song_editor_pointer), 1,
                    number_of_unpitched_note_columns));
    undo(song_editor_pointer);
  };
            ...
            {BadPasteRow({"", "not a mime",
             ...
             BadPasteRow(
                 {"[", UNPITCHED_NOTES_CELLS_MIME,
                  ...
                  "of input; expected '[', '{', or a literal"}),
             BadPasteRow(
                 {"[]", UNPITCHED_NOTES_CELLS_MIME, "Nothing to paste!"}),
